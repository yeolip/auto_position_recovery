...
class RecoveryAlgo():
    ...
    def m_ProjectUser3PosCoor(self, P_Ref1_Pts, P_Ref1_Base):
        print("//////////", funcname(), "//////////")
        # P_Ref1_Pts = np.asmatrix(
        # [[1261.29623616261,    239.985317917590,    51.5313160178364],
        # [1261.53702655582,    154.082122583680,    257.056595465577],
        ...
        # [260.6193,    418.9697,    126.6069]]
        # )
        Origin, Axis = self.m_MakeUser3PosAxis(P_Ref1_Base);
        ...
        # [0.003349159310558, 0.922253013439533, -0.386545326783471]]
        # )
        P_Result = (P_Ref1_Pts - np.tile(Origin, (P_Ref1_Pts.shape[0],1))) * Axis

        if (self.debugflag):
            print('ret',P_Result)
        return P_Result, Origin, Axis
    ...
    def m_MakeUser3PosAxis(self, P_Ref1_Disp):
        print("//////////",funcname(),"//////////")
        originPos = P_Ref1_Disp[2,:]
        ...
        XAxis = pos2 - pos3
        ZAxis = -np.cross(XAxis, pos1 - pos3)
...
class RecoveryCtrl():
    ...
    def check_available_regid_transform(self, tfirst, tsecond, tfirst_rest):
        ...
        if(len(tfirst) != len(tsecond)):
            checkOK = False
        if (len(tfirst) < C_MIN_VALUE___CALC or len(tsecond) < C_MIN_VALUE_RIGID_CALC):
            checkOK = False
    ...
    def calc_auto_recovery_3d_points(self, tDatas, tIdx, dictData, skipData, selectKey=None):
        print("//////////",funcname(),"//////////")
        retFlag = True
        retText = ""
        tdebug = 0
        ...
        print("")
        if selectKey is not None:
            retC, tIdx, dictData, retData, tSkipData = self.parsing_selected_data_by_tabType(tIdx, dictData, skipData, selectKey)
        else:
        ...
        for ipos, ititle in enumerate(ttitles):
            ...
            if(len(tdata_baseType) > 0):
                for i in range(0, len(tdata_baseType), tPatternCnt):
                    ...
                    retRelativePos, rr, tt = self.objCore.m_ProjectUser3PosCoor(tTarget, tBase)
    ...
    def parsing_selected_data_by_tabType(self, tIdx, dictData, skipData, selectKey=None):
        print("//////////",funcname(),"////////// ", tIdx)
        nCnt = 0
        nRet = True
        retData = ""
        ...
        if(tIdx == C_TAB1):
            if selectKey is not None:
                for key, value in dictData.items():
                    if (key.split('\t')[0] == selectKey and int(key.split('|')[1]) > 1):
